...
class HeadData:
    ...
    def split(self, fraction):
        ...
        data_train, data_test, idx_train, idx_test = train_test_split(self.data, self.indices, test_size=fraction, random_state=42)
...
def getRelationNersFromDocuments(documents):
    ...
    for doc in documents:
        for i in range(len(doc.relations)):
            relation_list = doc.relations[i]
            if relation_list == ['N']:
                continue
            for j in range(len(relation_list)):
                relation = relation_list[j]
                headidx = doc.heads[i][j]
                ner = doc.BIOs[headidx]
                if ner != 'O':
                    ec = ner[2:]
                bio_relation_ners.append(relation + '__' + 'B-' + ec)
                bio_relation_ners.append(relation + '__' + 'I-' + ec)
                ec_relation_ners.append(relation + '__' + ec)

    bio_relation_ners = list(set(bio_relation_ners))
    bio_relation_ners.sort()

    ec_relation_ners = list(set(ec_relation_ners))
    ec_relation_ners.sort()
    return bio_relation_ners, ec_relation_ners


def getEntity1Start(entity1_idx, bios):
    i = entity1_idx
    while i >= 0:
        if bios[i] == 'O':
            i -= 1
...
def getEntity1BIOId(entity, entityid, entity1_idx, bios, bio_tags):
    # default = bio_tags.index('O')
    # entity = ['O'] * len(bios)
    # entityid = [default] * len(bios)
    i = entity1_idx
    while i >=0 :
        if bios[i] == 'O':
            break
        if 'B-' in bios[i]:
            entityid[i] = bio_tags.index(bios[i])
            entity[i] = bios[i]
            break
        entityid[i] = bio_tags.index(bios[i])
        entity[i] = bios[i]
...
def getEntity2BIOId(entity1_idx, bios, relations, heads, relation_ners_tags):
    default = relation_ners_tags.index('N')
    entity = ['N'] * len(bios)
    entityid = [default] * len(bios)
    relation_list = relations[entity1_idx]
    head_list = heads[entity1_idx]
    if 'N' in relation_list:
        return entityid, entity
    ...
    for i in range(len(head_list)):
        ...
        while h >= 0:
            if bios[h] == 'O':
                break
            if 'B-' in bios[h]:
                entityid[h] = relation_ners_tags.index(r + '__' + bios[h])
                entity[h] = r + '__' + bios[h]
                break
            entityid[h] = relation_ners_tags.index(r + '__' + bios[h])
            entity[h] = r + '__' + bios[h]
            h -= 1
...
def getEntity1ECId(entity, entityid, entity1_idx, bios, ec_tags):
    # default = ec_tags.index('O')
    # entity = ['O'] * len(bios)
    # entityid = [default] * len(bios)
    i = entity1_idx
    while i >=0 :
        if bios[i] == 'O':
            break
        if 'B-' in bios[i]:
            entityid[i] = ec_tags.index(bios[i][2:])
            entity[i] = bios[i][2:]
            break
        entityid[i] = ec_tags.index(bios[i][2:])
        entity[i] = bios[i][2:]
...
def getEntity2ECId(entity1_idx, bios, relations, heads, relation_ners_tags):
    default = relation_ners_tags.index('N')
    entity = ['N'] * len(bios)
    entityid = [default] * len(bios)
    relation_list = relations[entity1_idx]
    ...
    for i in range(len(head_list)):
        ...
        while h >= 0:
            if bios[h] == 'O':
                break
            if 'B-' in bios[h]:
                entityid[h] = relation_ners_tags.index(r + '__' + bios[h][2:])
                entity[h] = r + '__' + bios[h][2:]
                break
            entityid[h] = relation_ners_tags.index(r + '__' + bios[h][2:])
            entity[h] = r + '__' + bios[h][2:]
            h -= 1
...
def getNegEntitys(bios, posEntitys):
    negEntitys = []
    for i in range(len(bios) - 1):
        if 'I-' in bios[i] and bios[i+1] == 'O' and i not in posEntitys:
            negEntitys.append(i)
        elif 'B-' in bios[i] and bios[i+1] == 'O' and i not in posEntitys:
            negEntitys.append(i)
        elif 'B-' in bios[i] and 'B-' in bios[i+1] and i not in posEntitys:
            negEntitys.append(i)
        elif 'I-' in bios[i] and 'B-' in bios[i+1] and i not in posEntitys:
            negEntitys.append(i)
    i += 1

    if 'I-' in bios[i] and i not in posEntitys:
        negEntitys.append(i)
    elif 'B-' in bios[i] and i not in posEntitys:
        negEntitys.append(i)
    return negEntitys
...
def readWordvectorsNumpy(wordvectorfile, isBinary=False):
    ...
    random.seed(123456)
    randomVec = [random.uniform(-np.sqrt(1. / len(wordvectors)), np.sqrt(1. / len(wordvectors))) for i in range(vectorsize)]
...
def printParameters(config):
    ...
    t.add_row(['Heads numbers ', config.num_heads])
...
def generator(data, m, config, train=False):
    ...
    for docIdx in range(len(data_copy.data)):
        ...
        branch = random()
        if branch >= 0.5:
            entity1_idx = choice(doc.posEntity1s)
            E1_end = entity1_idx
            E1_start = getEntity1Start(entity1_idx, doc.BIOs)
        else :
            entity1_idx = choice(doc.negEntity1s)
            E1_end = entity1_idx
            E1_start = getEntity1Start(entity1_idx, doc.BIOs)
        

        # print("------------end tag e1 and e2 in bio-----------------")

        #print (doc.jlabel_names)
        if config.ner_classes=="BIO":
            # default = config.dataset_set_bio_tags.index('O')
            # BIO_E1s = ['O'] * len(doc.BIOs)
            # BIO_E1_ids = [default] * len(doc.BIOs)

            # for e1idx in doc.Entity1s:
            #     getEntity1BIOId(BIO_E1s, BIO_E1_ids, e1idx, doc.BIOs, config.dataset_set_bio_tags)

            # BIO_E1_ids, BIO_E1s = getEntity1BIOId(entity1_idx, doc.BIOs, config.dataset_set_bio_tags)
            BIO_E2_ids, BIO_E2s = getEntity2BIOId(entity1_idx, doc.BIOs, doc.relations, doc.heads, config.dataset_set_bio_relation_ners)

            docs_batch_entity1_tags.append(doc.BIOs)##to do
            docs_batch_entity1_tags_ids.append(doc.BIO_ids)

            docs_batch_entity2_tags.append(BIO_E2s)##to do
            docs_batch_entity2_tags_ids.append(BIO_E2_ids)

        elif config.ner_classes=="EC":
            # default = config.dataset_set_ec_tags.index('O')
            # EC_E1s = ['O'] * len(doc.BIOs)
            # EC_E1_ids = [default] * len(doc.BIOs)

            # for e1idx in doc.Entity1s:
            #     getEntity1ECId(EC_E1s, EC_E1_ids, e1idx, doc.BIOs, config.dataset_set_ec_tags)

            # EC_E1_ids, EC_E1s= getEntity1ECId(entity1_idx, doc.BIOs, config.dataset_set_ec_tags)
            EC_E2_ids, EC_E2s= getEntity2ECId(entity1_idx, doc.BIOs, doc.relations, doc.heads, config.dataset_set_ec_relation_ners)

            docs_batch_entity1_tags.append(doc.ecs)##to do
            docs_batch_entity1_tags_ids.append(doc.ec_ids)

            docs_batch_entity2_tags.append(EC_E2s)##to do
            docs_batch_entity2_tags_ids.append(EC_E2_ids)
            

        docs_batch_tokens.append(doc.tokens)
        

        docs_batch_k1s.append(E1_start)
        docs_batch_k2s.append(E1_end)
    ...
    for bIdx in range(len(batches_embeddingIds)):  # 131
        ...
        batch_bio = np.asarray(batches_BIO[bIdx]) ## (batchsize, 36)
...
def get_true_triples(names, relations, indices):
    triples = [] ## list of triple ((i, j), r, (m, n)) e.g. e1=St. Albans; r=Located_In; e2=Vt.
    lg = len(names)
    for i in range(lg):
        if relations[i] == ['N']:
            continue
            ...
            inds = indices[i]
            e1 = get_word(names, i)
            e2s = get_e2s(names, inds)
            assert len(e2s) == len(rels)
            for r, e2 in zip(rels, e2s):
                t = (e1, r, e2)
                triples.append(t)
    return triples


def get_word(names, i):
    tag = names[i]
    tag_class, tag_type = tag.split('-')[0], tag.split('-')[-1]
    j = i
    while j >= 0:
        if tag_type not in names[j] or names[j] == 'O':
            break
        j -= 1
    return (j+1, i) 
        

def get_e2s(names, inds):
    e2s = []
    for i in inds:
        e2 = get_word(names, i)
        e2s.append(e2)
...
def getNerAndRel(doc):
    ...
    Heads = doc.heads
    true_triples = get_true_triples(Bios, Relations, Heads)
    true_ners = get_true_ners(Bios)

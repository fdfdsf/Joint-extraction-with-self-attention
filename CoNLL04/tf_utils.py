...
class model:
    ...
    def __init__(self, config, emb_mtx, sess):
        ...
        self.BIO = tf.placeholder(tf.string, [None, None])  # [ batch_size  *   max_sequence]
    ...
    def train(self, train_data, operations, iter):
        ...
        for x_train in utils.generator(train_data, operations.m_op, self.config, train=True):
            _, val, m_train, transition_params1, entity1Scores, predEntity1, transition_params2, entity2Scores, predEntity2 = self.sess.run(
                                            ...
                                            operations.transition_params1, operations.entity1Scores, operations.predEntity1,
                                            operations.transition_params2, operations.entity2Scores, operations.predEntity2], feed_dict=x_train)
    ...
    def evaluate(self, eval_data, operations, set):
        ...
        for x_dev, doc in utils.generator(eval_data, operations.m_op, self.config, train=False):
            ...
            entity1Scores, predEntity1s = self.sess.run([operations.entity1Scores, operations.predEntity1], feed_dict=x_dev)
    ...
    def model1(self, lstm_output, reuse=False):
        ...
        with tf.variable_scope("model1_computation", reuse=reuse):
            ...
            if self.config.ner_classes == "EC":
                ...
                entity1Scores = self.getNerScores1(entity1_input, len(self.config.dataset_set_ec_tags),
                                              dropout_keep_in_prob=self.dropout_fcl_ner_keep, reuse=reuse)
                ...
                entity1Scores = self.getNerScores1(entity1_input, len(self.config.dataset_set_bio_tags),
                                              dropout_keep_in_prob=self.dropout_fcl_ner_keep, reuse=reuse)
                label_matrix = tf.get_variable(name="label_embeddings", dtype=tf.float32,
            ...
            # entity1Scores = tf.Print(entity1Scores, [tf.shape(entity1_ids), entity1_ids, tf.shape(entity1Scores)], 'entity1_ids:  ', summarize=1000)
            ...
            if self.config.ner_loss == "crf":
                ...
                return lossEntity1, transition_params, entity1Scores, label_matrix, predEntity1
                ...
                return lossEntity1, entity1Scores, label_matrix, predEntity1


    def model2(self, lstm_output, predEntity1, entity1Scores, label_matrix, reuse=False):
        import tensorflow as tf
        ...
        with tf.variable_scope("model2_computation", reuse=reuse):
            ...
            if self.config.ner_classes == "EC":
                ...
                entity2Scores = self.getNerScores2(entity2_input, len(self.config.dataset_set_ec_relation_ners),
                                              dropout_keep_in_prob=self.dropout_fcl_ner_keep, reuse=reuse)
            elif self.config.ner_classes == "BIO":

                entity2Scores = self.getNerScores2(entity2_input, len(self.config.dataset_set_bio_relation_ners),
                                              dropout_keep_in_prob=self.dropout_fcl_ner_keep, reuse=reuse)
    ...
    def run(self):
        ...
        if self.config.ner_loss == "crf":
            lossEntity1, transition_params1, entity1Scores, label_matrix, predEntity1 = self.model1(lstm_output, reuse=False)
            lossEntity2, transition_params2, entity2Scores, predEntity2 = self.model2(lstm_output, predEntity1, entity1Scores, label_matrix, reuse=False)

        elif self.config.ner_loss == "softmax":
            lossEntity1, entity1Scores, label_matrix, predEntity1 = self.model1(lstm_output, reuse=False)
            lossEntity2, entity2Scores, predEntity2 = self.model2(lstm_output, predEntity1, entity1Scores, label_matrix, reuse=False)
        ...
        if self.config.ner_loss == "crf":
            lossEntity1_per, transition_params1_per, entity1Scores_per, label_matrix_per, _ = self.model1(perturb_inputs, reuse=True)
            lossEntity2_per, _, _, _ = self.model2(perturb_inputs, transition_params1_per, entity1Scores_per, label_matrix_per, reuse=True)

        elif self.config.ner_loss == "softmax":
            lossEntity1_per, entity1Scores_per, label_matrix_per, _ = self.model1(perturb_inputs, reuse=True)
            lossEntity2_per, _, _ = self.model2(perturb_inputs, None, entity1Scores_ner, label_matrix_ner, reuse=True)
